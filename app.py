...
from common import IST, yesterday, today, logger, fixed_response_dict, round_spot
from contracts import get_req_contracts
from db_ops import DBHandler
from pydantic import BaseModel
from passlib.context import CryptContext
...
class ServiceApp:
    ...
    def fetch_straddle_cluster(self, symbol: str = Query(), expiry: date = Query(), st_cnt: int = Query(default=10),
        ...
        df.to_excel(f"{symbol}_{expiry}.xlsx", index=False)
        today_max_ts = today_df['ts'].unique().max()
        ...
        rounded_spot = round_spot(symbol = symbol, spot = spot_today_max_ts[0])
        logger.info(f"\n{symbol}_{expiry} rounded spot is \n{rounded_spot} \t type is {type(rounded_spot)}")
